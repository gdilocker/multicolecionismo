/*
  ...
  2. Segurança
    ...
    - Admin users have full access regardless of ownership
...
DROP POLICY IF EXISTS "Users can insert own products" ON store_products;
DROP POLICY IF EXISTS "Users can update own products" ON store_products;
DROP POLICY IF EXISTS "Users can delete own products" ON store_products;
DROP POLICY IF EXISTS "Admin full access to store products" ON store_products;

-- Policy: Admin users have full access
CREATE POLICY "Admin full access to store products"
  ON store_products
  FOR ALL
  TO authenticated
  USING (
