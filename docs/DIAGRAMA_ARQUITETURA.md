# 🎨 DIAGRAMA DA ARQUITETURA - TheRichClub

**Data:** 2025-11-09
**Status:** Visualização Completa do Sistema

---

## 🌐 VISÃO GERAL DO FLUXO

```
┌─────────────────────────────────────────────────────────────────────┐
│                           USUÁRIO FINAL                              │
│                                                                      │
│  1. Acessa: therichclub.com                                         │
│  2. Vê: "Busque seu domínio .multicolecionismo.social"                             │
│  3. Digita: "maria"                                                 │
└──────────────────────────┬──────────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────────┐
│                      FRONTEND (React + Vite)                         │
│                      Hospedado em: Netlify                          │
│                                                                      │
│  ┌─────────────────────────────────────────────────────────┐       │
│  │  Home.tsx                                                │       │
│  │  • Input: [maria    ] .multicolecionismo.social                         │       │
│  │  • Monta FQDN: "maria.multicolecionismo.social"                         │       │
│  │  • Chama Edge Function                                  │       │
│  └─────────────────────────────────────────────────────────┘       │
└──────────────────────────┬──────────────────────────────────────────┘
                           │
                           │ POST /functions/v1/domains
                           │ { action: 'check', fqdn: 'maria.multicolecionismo.social' }
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────────┐
│                   SUPABASE EDGE FUNCTIONS                            │
│                                                                      │
│  ┌─────────────────────────────────────────────────────────┐       │
│  │  domains/index.ts                                        │       │
│  │                                                          │       │
│  │  1. Recebe requisição                                   │       │
│  │  2. Verifica no banco de dados                          │       │
│  │  3. Retorna status                                      │       │
│  └─────────────────────────────────────────────────────────┘       │
└──────────────────────────┬──────────────────────────────────────────┘
                           │
                           │ Query SQL
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────────┐
│                   SUPABASE DATABASE (PostgreSQL)                     │
│                                                                      │
│  ┌──────────────────┐  ┌──────────────────┐  ┌──────────────────┐ │
│  │    domains       │  │ premium_domains  │  │  user_profiles   │ │
│  │                  │  │                  │  │                  │ │
│  │ • fqdn           │  │ • fqdn           │  │ • display_name   │ │
│  │ • customer_id    │  │ • price_usd      │  │ • bio            │ │
│  │ • domain_type    │  │ • required_plan  │  │ • profile_image  │ │
│  │ • status         │  │ • is_available   │  │ • domain_id      │ │
│  └──────────────────┘  └──────────────────┘  └──────────────────┘ │
│                                                                      │
│  Busca: SELECT * FROM domains WHERE fqdn = 'maria.multicolecionismo.social'        │
│                                                                      │
│  Resultado: NULL (não existe) = AVAILABLE                          │
│  Resultado: {customer_id: '...'} = UNAVAILABLE                     │
│  Resultado: premium_domains match = PREMIUM                        │
└──────────────────────────┬──────────────────────────────────────────┘
                           │
                           │ Retorna resultado
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────────┐
│                      FRONTEND (Home.tsx)                             │
│                                                                      │
│  ┌─────────────────────────────────────────────────────────┐       │
│  │  ✅ AVAILABLE                                            │       │
│  │                                                          │       │
│  │  maria.multicolecionismo.social                                         │       │
│  │  Domínio disponível para registro!                     │       │
│  │                                                          │       │
│  │  [📋 Ver Planos]                                        │       │
│  └─────────────────────────────────────────────────────────┘       │
└──────────────────────────┬──────────────────────────────────────────┘
                           │
                           │ Usuário clica "Ver Planos"
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────────┐
│                      /pricing (Pricing.tsx)                          │
│                                                                      │
│  ┌─────────┬─────────┬─────────┬─────────┐                        │
│  │ Starter │  Prime  │  Elite  │ Supreme │                        │
│  │ GRÁTIS  │ $19/mês │ $70/mês │$300/mês │                        │
│  └─────────┴─────────┴─────────┴─────────┘                        │
│                                                                      │
│  Usuário escolhe plano → /checkout                                 │
└──────────────────────────┬──────────────────────────────────────────┘
                           │
                           │ Pagamento via PayPal
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────────┐
│                      PAYPAL API                                      │
│                                                                      │
│  1. Cria subscription                                               │
│  2. Usuário paga                                                    │
│  3. PayPal webhook → Supabase Edge Function                        │
└──────────────────────────┬──────────────────────────────────────────┘
                           │
                           │ Webhook confirma pagamento
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────────┐
│                   SUPABASE: Criação Automática                       │
│                                                                      │
│  1. INSERT INTO customers (user_id, email, ...)                    │
│  2. INSERT INTO subscriptions (customer_id, plan, status)          │
│  3. INSERT INTO domains (fqdn, customer_id, status)                │
│  4. INSERT INTO user_profiles (user_id, domain_id, ...)            │
│                                                                      │
│  Status: PERFIL CRIADO! ✅                                          │
└──────────────────────────┬──────────────────────────────────────────┘
                           │
                           │ Usuário logado → Dashboard
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────────┐
│                      /dashboard (Dashboard.tsx)                      │
│                                                                      │
│  Bem-vindo, Maria! 👋                                               │
│                                                                      │
│  Seus domínios:                                                     │
│  • maria.multicolecionismo.social [ATIVO]                                          │
│                                                                      │
│  [✏️ Editar Perfil] [📊 Estatísticas] [⚙️ Configurações]          │
└──────────────────────────┬──────────────────────────────────────────┘
                           │
                           │ Edita perfil
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────────┐
│                  /profile-manager (ProfileManager.tsx)               │
│                                                                      │
│  ┌──────────────────────────────────────────────────────┐          │
│  │  • Upload foto de perfil                             │          │
│  │  • Adiciona bio                                      │          │
│  │  • Cria links personalizados                         │          │
│  │  • Configura loja (se habilitado)                    │          │
│  │  • Customiza cores/tema                              │          │
│  └──────────────────────────────────────────────────────┘          │
│                                                                      │
│  Salva → UPDATE user_profiles SET ...                              │
└──────────────────────────┬──────────────────────────────────────────┘
                           │
                           │ PERFIL PÚBLICO AGORA DISPONÍVEL!
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────────┐
│              PERFIL PÚBLICO: /u/maria (PublicProfile.tsx)            │
│                                                                      │
│  URL no navegador: https://therichclub.com/u/maria                 │
│                                                                      │
│  ┌──────────────────────────────────────────────────────┐          │
│  │           [Foto]                                     │          │
│  │      maria.multicolecionismo.social                                  │          │
│  │      Identidade Digital Exclusiva                    │          │
│  │                                                       │          │
│  │      Bio: "Empreendedora digital..."                │          │
│  │                                                       │          │
│  │      🔗 Links:                                       │          │
│  │      • Instagram                                     │          │
│  │      • LinkedIn                                      │          │
│  │      • Website                                       │          │
│  │                                                       │          │
│  │      🛍️ Loja (se habilitado)                        │          │
│  │      📱 Posts Sociais (se habilitado)               │          │
│  └──────────────────────────────────────────────────────┘          │
│                                                                      │
│  Header mostra: "maria.multicolecionismo.social"                                   │
│  Mas URL real é: therichclub.com/u/maria                           │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 🔄 FLUXO DE DADOS DETALHADO

### **1. Verificação de Domínio**

```
Usuario        Frontend           Edge Function      Database
   │              │                     │               │
   │─┐ Digite    │                     │               │
   │ │"maria"    │                     │               │
   │<┘           │                     │               │
   │             │                     │               │
   │─────────────>│ handleSearch()    │               │
   │             │                     │               │
   │             │─────────────────────>│ POST         │
   │             │  action: 'check'    │ /domains     │
   │             │  fqdn: 'maria...'   │               │
   │             │                     │               │
   │             │                     │──────────────>│
   │             │                     │ SELECT * FROM │
   │             │                     │ domains WHERE │
   │             │                     │ fqdn = '...'  │
   │             │                     │               │
   │             │                     │<──────────────│
   │             │                     │ NULL          │
   │             │                     │ (disponível)  │
   │             │                     │               │
   │             │<─────────────────────│ { status:    │
   │             │   JSON Response      │ 'AVAILABLE' } │
   │             │                     │               │
   │<─────────────│ setAvailable(true)│               │
   │  ✅ Mostra  │                     │               │
   │  "Disponível"│                    │               │
```

### **2. Registro e Pagamento**

```
Usuario    Frontend    PayPal API    Webhook    Database
   │          │            │            │           │
   │──────────>│ Escolhe   │            │           │
   │  Prime    │ plano     │            │           │
   │          │            │            │           │
   │<──────────│ Redirect  │            │           │
   │  PayPal   │ checkout  │            │           │
   │          │            │            │           │
   │──────────────────────>│ Cria       │           │
   │          │  Login +   │ subscription│           │
   │          │  Paga      │            │           │
   │          │            │            │           │
   │          │            │────────────>│ Confirma  │
   │          │            │  Webhook   │ pagamento │
   │          │            │            │           │
   │          │            │            │──────────>│
   │          │            │            │ INSERT    │
   │          │            │            │ customer, │
   │          │            │            │ subscription│
   │          │            │            │ domain    │
   │          │            │            │ profile   │
   │          │            │            │           │
   │<─────────────────────────────────────────────────│
   │          │  Email confirmação                   │
   │          │  "maria.multicolecionismo.social criado!"            │
```

### **3. Acesso ao Perfil Público**

```
Visitante      Frontend         Database        Profile
   │              │                 │              │
   │──────────────>│ Acessa:        │              │
   │ /u/maria     │ PublicProfile  │              │
   │              │                 │              │
   │              │─────────────────>│ SELECT *   │
   │              │ useParams()     │ FROM        │
   │              │ username='maria'│ user_profiles│
   │              │                 │ JOIN domains │
   │              │                 │              │
   │              │<─────────────────│ {          │
   │              │   Profile data  │   display_name│
   │              │                 │   bio,      │
   │              │                 │   links     │
   │              │                 │ }           │
   │              │                 │              │
   │<──────────────│ Renderiza      │              │
   │  Perfil      │ maria.multicolecionismo.social  │              │
   │  completo    │ (header visual) │              │
```

---

## 🏗️ ARQUITETURA DE COMPONENTES

```
┌─────────────────────────────────────────────────────────────┐
│                        APP.TSX                               │
│                     (Router Principal)                       │
└────────────────────┬────────────────────────────────────────┘
                     │
        ┌────────────┼────────────┬──────────────┬──────────┐
        │            │            │              │          │
        ▼            ▼            ▼              ▼          ▼
    ┌─────┐    ┌─────────┐  ┌──────────┐  ┌──────────┐ ┌────┐
    │Home │    │Pricing  │  │Dashboard │  │  /u/:    │ │... │
    │.tsx │    │.tsx     │  │.tsx      │  │username  │ │    │
    └─────┘    └─────────┘  └──────────┘  └──────────┘ └────┘
       │                          │              │
       │                          ▼              │
       │                    ┌──────────┐         │
       │                    │Profile   │         │
       │                    │Manager   │         │
       │                    │.tsx      │         │
       │                    └──────────┘         │
       │                          │              │
       ▼                          ▼              ▼
   ┌───────────────────────────────────────────────────┐
   │              SHARED COMPONENTS                     │
   │                                                    │
   │  • Header.tsx                                     │
   │  • Footer.tsx                                     │
   │  • LoginModalSimple.tsx                           │
   │  • SuccessModal.tsx                               │
   │  • DomainSearch.tsx                               │
   │  • LinkEditor.tsx                                 │
   │  • SocialButtonsEditor.tsx                        │
   │  • StoreManager.tsx (condicional)                 │
   │  • SocialFeed.tsx (condicional)                   │
   └───────────────────────────────────────────────────┘
```

---

## 🗄️ ESTRUTURA DO BANCO DE DADOS

```
┌──────────────────────────────────────────────────────────────┐
│                     auth.users (Supabase Auth)                │
│  • id (UUID)                                                  │
│  • email                                                      │
│  • encrypted_password                                         │
└────────────────────────┬─────────────────────────────────────┘
                         │
                         │ 1:1
                         ▼
┌──────────────────────────────────────────────────────────────┐
│                      customers                                │
│  • id (UUID)                                                  │
│  • user_id (FK → auth.users)                                 │
│  • email                                                      │
│  • full_name                                                  │
│  • role (user | reseller | admin)                            │
└────────────────────────┬─────────────────────────────────────┘
                         │
                         │ 1:N
                         ▼
┌──────────────────────────────────────────────────────────────┐
│                      subscriptions                            │
│  • id (UUID)                                                  │
│  • customer_id (FK → customers)                              │
│  • plan_code (starter | prime | elite | supreme)             │
│  • status (active | suspended | cancelled)                   │
│  • trial_ends_at                                             │
└────────────────────────┬─────────────────────────────────────┘
                         │
                         │ 1:N (conforme limite do plano)
                         ▼
┌──────────────────────────────────────────────────────────────┐
│                        domains                                │
│  • id (UUID)                                                  │
│  • fqdn (TEXT UNIQUE) → "maria.multicolecionismo.social"                     │
│  • customer_id (FK → customers)                              │
│  • domain_type (regular | premium)                           │
│  • status (active | suspended | cancelled)                   │
└────────────────────────┬─────────────────────────────────────┘
                         │
                         │ 1:1
                         ▼
┌──────────────────────────────────────────────────────────────┐
│                     user_profiles                             │
│  • id (UUID)                                                  │
│  • user_id (FK → auth.users)                                 │
│  • domain_id (FK → domains)                                  │
│  • display_name                                              │
│  • bio                                                       │
│  • profile_image                                             │
│  • background_image                                          │
│  • custom_css                                                │
│  • show_store (BOOLEAN)                                      │
│  • show_social (BOOLEAN)                                     │
└────────────────────────┬─────────────────────────────────────┘
                         │
        ┌────────────────┼────────────────┐
        │                │                │
        ▼                ▼                ▼
   ┌────────┐      ┌──────────┐    ┌──────────┐
   │ links  │      │ products │    │  posts   │
   │        │      │ (store)  │    │ (social) │
   └────────┘      └──────────┘    └──────────┘
```

---

## 🌍 DNS E ROTEAMENTO

### **NÃO EXISTE (Importante!):**

```
❌ DNS Wildcard:
   *.multicolecionismo.social → NÃO CONFIGURADO

❌ Subdomínios Reais:
   maria.multicolecionismo.social → NÃO É UM DNS REAL
   vip.multicolecionismo.social → NÃO É UM DNS REAL
```

### **O QUE EXISTE:**

```
✅ DNS Único:
   therichclub.com → A Record → [IP Netlify]
   www.therichclub.com → CNAME → site.netlify.app

✅ React Router:
   /                 → Home.tsx
   /pricing          → Pricing.tsx
   /dashboard        → Dashboard.tsx
   /u/:username      → PublicProfile.tsx
   /u/maria          → Busca "maria.multicolecionismo.social" no banco
```

### **Fluxo de Acesso:**

```
Usuário digita: therichclub.com/u/maria
                       │
                       ▼
               DNS resolve para IP
                       │
                       ▼
               Netlify serve app React
                       │
                       ▼
         React Router captura /u/maria
                       │
                       ▼
         PublicProfile.tsx recebe username="maria"
                       │
                       ▼
      Busca no banco: domains.fqdn = "maria.multicolecionismo.social"
                       │
                       ▼
              Renderiza perfil com header:
           "maria.multicolecionismo.social" (só visual!)
```

---

## 🔐 SEGURANÇA E RLS

```
┌─────────────────────────────────────────────────────────┐
│                    REQUISIÇÃO                            │
│                                                          │
│  1. Usuário faz request                                 │
│  2. Token JWT no header                                 │
│  3. Supabase valida token                               │
│  4. auth.uid() extraído do JWT                          │
└────────────────────┬────────────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────────────┐
│                ROW LEVEL SECURITY (RLS)                  │
│                                                          │
│  Para cada query SQL, Supabase adiciona WHERE:          │
│                                                          │
│  SELECT * FROM user_profiles                            │
│  WHERE ...                                              │
│    AND (auth.uid() = user_id)  ← RLS Policy            │
│                                                          │
│  Resultado: Só retorna dados que o usuário pode ver    │
└─────────────────────────────────────────────────────────┘

EXEMPLOS:

1. Leitura Pública:
   SELECT * FROM user_profiles
   WHERE domain.status = 'active'  ← Qualquer um vê

2. Edição Restrita:
   UPDATE user_profiles SET bio = '...'
   WHERE id = '...'
   AND auth.uid() = user_id  ← Só o dono edita

3. Admin Override:
   SELECT * FROM customers
   WHERE role = 'admin' OR customer_id = auth.uid()
```

---

## 📊 PLANOS E LIMITES

```
┌──────────┬─────────┬────────────┬──────────────────────┐
│  Plano   │  Preço  │  Domínios  │      Features        │
├──────────┼─────────┼────────────┼──────────────────────┤
│ Starter  │  GRÁTIS │     1      │ Regular only         │
│          │ 14 dias │  regular   │ Perfil básico        │
│          │         │            │                      │
├──────────┼─────────┼────────────┼──────────────────────┤
│ Prime    │ $19/mês │     3      │ Regulares only       │
│          │         │ regulares  │ Perfil + Links       │
│          │         │            │ Loja opcional        │
│          │         │            │                      │
├──────────┼─────────┼────────────┼──────────────────────┤
│ Elite    │ $70/mês │    10      │ Regular + Premium    │
│          │         │ + premium  │ Tudo do Prime        │
│          │         │            │ + Domínios premium   │
│          │         │            │ + Social network     │
│          │         │            │                      │
├──────────┼─────────┼────────────┼──────────────────────┤
│ Supreme  │$300/mês │    50      │ Tudo do Elite        │
│          │+$5k fee │ + premium  │ + Prioridade         │
│          │         │            │ + Suporte VIP        │
└──────────┴─────────┴────────────┴──────────────────────┘

VALIDAÇÃO NO CÓDIGO:

if (userPlan === 'starter' && domainCount >= 1) {
  throw new Error('Limite atingido. Upgrade para Prime.');
}

if (isPremiumDomain && !['elite', 'supreme'].includes(userPlan)) {
  throw new Error('Domínio premium requer Elite ou Supreme.');
}
```

---

## 🔄 EDGE FUNCTIONS (Backend)

```
┌──────────────────────────────────────────────────────────┐
│              PRINCIPAIS EDGE FUNCTIONS                    │
└──────────────────────────────────────────────────────────┘

1. domains/index.ts
   • Verifica disponibilidade de domínios
   • Retorna: AVAILABLE, UNAVAILABLE, PREMIUM

2. paypal-create-subscription/index.ts
   • Cria assinatura no PayPal
   • Retorna: approval_url

3. paypal-webhook/index.ts
   • Recebe confirmação de pagamento
   • Cria customer, subscription, domain, profile

4. auto-create-profile/index.ts
   • Trigger: Novo usuário cadastrado
   • Cria profile inicial automaticamente

5. verify-link-security/index.ts
   • Valida links com Google Safe Browsing
   • Marca links suspeitos como unsafe

6. domain-lifecycle-cron/index.ts
   • Roda diariamente
   • Suspende domínios com pagamento atrasado
   • Libera domínios cancelados após grace period

FLUXO TÍPICO:

Frontend → Edge Function → Database → Response → Frontend
   |            |              |          |          |
   POST      Valida JWT    Apply RLS   JSON      Update UI
          Rate Limit      Run Query   Result    Show Data
          Sanitize Input  Audit Log
```

---

## 📱 RESPONSIVE DESIGN

```
┌─────────────────────────────────────────────────────────┐
│                  MOBILE (< 768px)                        │
│                                                          │
│  ┌────────────────────────────────────────────┐        │
│  │  ☰  therichclub.com           [Login]      │        │
│  ├────────────────────────────────────────────┤        │
│  │                                             │        │
│  │  [ username     ] .multicolecionismo.social                │        │
│  │        [🔍 Buscar]                         │        │
│  │                                             │        │
│  │  ✅ maria.multicolecionismo.social                         │        │
│  │  Disponível!                               │        │
│  │  [📋 Ver Planos]                           │        │
│  │                                             │        │
│  └────────────────────────────────────────────┘        │
└─────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────┐
│                 DESKTOP (>= 1024px)                      │
│                                                          │
│  ┌────────────────────────────────────────────────────┐ │
│  │  therichclub.com    [Home][Pricing][Login]         │ │
│  ├────────────────────────────────────────────────────┤ │
│  │                                                     │ │
│  │     Seu domínio .multicolecionismo.social                          │ │
│  │                                                     │ │
│  │     [ username      ] .multicolecionismo.social  [🔍 Buscar]      │ │
│  │                                                     │ │
│  │     ✅ maria.multicolecionismo.social está disponível!             │ │
│  │                                                     │ │
│  │     [📋 Ver Planos]                                │ │
│  │                                                     │ │
│  └────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────┘
```

---

## ✅ RESUMO VISUAL

```
┌─────────────────────────────────────────────────────────────┐
│              O QUE É TheRichClub?                            │
│                                                              │
│  ❌ NÃO É:                      ✅ É:                       │
│  • Registrador de .com         • Plataforma de perfis      │
│  • DNS wildcard real           • Sistema de licenciamento  │
│  • Subdomínios reais           • React Router + Banco      │
│  • Múltiplos servidores        • Display ".multicolecionismo.social" fake  │
│                                 • URL real: /u/:username   │
│                                                              │
│  USUÁRIO VÊ:                   SISTEMA USA:                │
│  maria.multicolecionismo.social                therichclub.com/u/maria     │
│  (só visual!)                  (URL real!)                 │
└─────────────────────────────────────────────────────────────┘
```

---

**Link para Diagrama Online (opcional):**

Você pode criar um diagrama interativo usando:
- **Excalidraw:** https://excalidraw.com
- **Draw.io:** https://app.diagrams.net
- **Mermaid:** (já em markdown)

---

**Última Atualização:** 2025-11-09
